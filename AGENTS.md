# AGENTS.md — Aturan Project Money Tracker Personal
...
- **Versi app:** `pubspec.yaml:4` (`1.0.0+1`), SDK `^3.12.2`
- **Stack terkunci:** Flutter Material 3 + Drift 6 tabel + Riverpod + go_router 5 tab + fl_chart — lihat `PRD.md:40-52`
- **DB:** `lib/database/app_database.dart:14-26` (`money_tracker.db`, FK ON)
- **Bahasa & mata uang:** Indonesia, IDR (Rp), locale `id_ID` — `lib/main.dart:8`

## 2. ATURAN MUTLAK: Wajib Update PRD.md Tiap Selesai Fase

> **PERINTAH MUTLAK UNTUK AI:** Setiap selesai SATU FASE (Fase 0-6 di `PRD.md:394-402` §16 Roadmap), AI **WAJIB** update `PRD.md` sebelum menyatakan fase selesai. **Dilarang skip.**
...
- Material 3, `google_fonts` Inter, warna di `lib/core/theme/app_colors.dart:5-58`
- Selalu rujuk file dengan format `path:line_number` saat menyebut kode
- Verifikasi via eksekusi, bukan asumsi

## 5. Reuse ke App Berbeda

- Master prompt: `Prompt.md:1-207` — copy `## Prompt Siap Copy-Paste`, ganti `{{VARIABEL}}`, paste ke chat baru
- Template mentah: `templates/PRD.template.md` + `templates/AGENTS.template.md`
- Command: `/new-app APP_NAME | APP_IDEA | PROJECT_PATH` — lihat `.opencode/command/new-app.md`
